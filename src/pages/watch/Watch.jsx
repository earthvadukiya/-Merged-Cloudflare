                ...
                <iframe
                  ...
                  sandbox={
                    playerAdBlockEnabled
                      ? // Megaplay refuses to run if it detects a *restrictive*
                        // sandbox (it checks that popups are blocked). So we give
                        // it the tokens it expects to see (allow-popups +
                        // allow-top-navigation-BY-USER-ACTIVATION) which lets the
                        // player run, while still WITHHOLDING the unrestricted
                        // `allow-top-navigation` + `allow-modals` that auto-redirect
                        // ad scripts rely on. This kills the silent pop-under /
                        // auto-redirect ads while keeping megaplay playable.
