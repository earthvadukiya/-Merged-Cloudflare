...
import { resilientGet } from "@/src/lib/resilientFetch";
...
function normalizeHomeResponse(responseData = {}) {
  const r = responseData?.data || responseData?.results || responseData || {};
...
export default async function getHomeInfo() {
  const apiUrl = HOME_API;

  // Instant path: fresh browser cache (avoids any network at all on repeat visits).
  ...
  // Resilient path: fresh Supabase snapshot -> live API (retry/backoff) ->
  // stale snapshot -> stale localStorage. Makes the home feed unbreakable.
  const homeData = await resilientGet({
    cacheKey: "home",
    maxAgeMs: CACHE_TTL,
    fetcher: async () => {
    ...
    fallback: cached?.data || EMPTY_HOME_DATA,
  });

  if (homeData && homeData !== EMPTY_HOME_DATA) saveLocalCache(homeData);
  return homeData || EMPTY_HOME_DATA;
}
