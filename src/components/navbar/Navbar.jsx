...
function Navbar() {
  ...
  const { isAuthed, displayName, openAuth, signOut } = useAuth();

  // Close the user dropdown when navigating.
  useEffect(() => {
        ...
        <div className="max-w-[980px] mx-auto px-4 pointer-events-auto">
          ...
          >
            ...
            <div className="flex items-center gap-3">
              ...
              {/* Auth: login button (guest) or avatar menu (logged in) */}
              {!isAuthed ? (
                <button
                  onClick={() => openAuth("login")}
                  className="flex items-center justify-center gap-2 h-8 px-3 rounded-full text-white font-semibold text-sm bg-white/10 border border-white/15 hover:bg-white/20 transition"
                  aria-label="Log in"
                >
                ...
                <div className="relative">
                  <button
                    ...
                    className="w-8 h-8 rounded-full bg-gradient-to-br from-[#a855f7] to-[#7c3aed] flex items-center justify-center text-white font-bold uppercase shadow-lg shadow-[#a855f7]/25 hover:brightness-110 transition"
                  ...
                  {userMenuOpen && (
                    <>
                      ...
                      />
                      <div className="absolute right-0 mt-2 w-52 z-[10001] rounded-2xl border border-white/10 bg-[#0c0c12] shadow-[0_20px_70px_rgba(0,0,0,0.8)] overflow-hidden">
                        ...
                        >
                          <FontAwesomeIcon icon={faGaugeHigh} className="text-[#c084fc]" />
                          My Dashboard
                        </button>
                        <button
                          onClick={async () => {
