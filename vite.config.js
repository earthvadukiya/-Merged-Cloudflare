...
export default defineConfig({
  ...
  // Strip console.* and debugger statements from the production bundle. This
  // removes the leftover debug logs that would otherwise reveal internal data
  // shapes / endpoints in the browser console.
  esbuild: {
    drop: ["debugger"],
    pure: ["console.log", "console.info", "console.debug"],
  },
  build: {
    // Never emit source maps in production — they would map the minified bundle
    // straight back to readable source, defeating the point of minifying.
    sourcemap: false,
    // Aggressive minification (default esbuild) keeps the JS hard to read.
    minify: "esbuild",
