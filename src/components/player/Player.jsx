...
import {
  ...
  getResumeTime,
  clearPosition,
} from "@/src/utils/playbackProgress.utils";
...
export default function Player({
  ...
  useEffect(() => {
    ...
    art.on("ready", () => {
      ...
      // Keyed per anime+episode so each episode keeps its own resume point.
      const pKey = progressKey(animeInfo?.id, episodeNum);
      const resumeAt = getResumeTime(pKey);
      if (resumeAt > 0 && resumeAt < (art.duration || Infinity)) {
        art.currentTime = resumeAt;
        try {
          const mins = Math.floor(resumeAt / 60);
          const secs = String(Math.floor(resumeAt % 60)).padStart(2, "0");
          art.notice.show = `Resuming from ${mins}:${secs}`;
        } catch {
          /* notice plugin unavailable — silent resume is fine */
        }
      }

      // Continuously remember the latest position. We throttle the localStorage
      // write to once every ~5s of playback so we never thrash the disk, but the
      // in-memory ref updates every tick so unmount/pause always saves the exact
      // second.
      let lastSaved = 0;
      art.on("video:timeupdate", () => {
        ...
        if (t - lastSaved >= 5 || t < lastSaved) {
          lastSaved = t;
          savePosition(pKey, t, art.duration || 0);
        }
      });
      // Save immediately when the viewer pauses or the page is hidden.
      art.on("video:pause", () => savePosition(pKey, art.currentTime, art.duration || 0));
      // When an episode finishes, drop its resume point so next time starts fresh.
    ...
    // Persist position if the tab is closed/hidden mid-playback (unmount won't
    // always fire on a hard close, but these events do).
    const flushPosition = () => {
      try {
        if (art && Number.isFinite(art.currentTime)) {
          savePosition(progressKey(animeInfo?.id, episodeNum), art.currentTime, art.duration || 0);
        }
      } catch {
        /* ignore */
      }
    ...
    return () => {
      ...
      try {
        ...
        // One entry per anime (latest episode only), newest first, capped at 10.
        const filtered = continueWatching.filter(
          (item) => item.data_id !== newEntry.data_id && String(item.id) !== String(newEntry.id)
        );
        filtered.unshift(newEntry);
        localStorage.setItem("continueWatching", JSON.stringify(filtered.slice(0, 10)));
      } catch (err) {
